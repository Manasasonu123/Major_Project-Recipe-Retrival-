...
function History() {
  ...
  const showPredictedFood = async (userId) => {
    try {
      const response = await fetch(
        `http://localhost:8000/users/show-prediction?userId=${userId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const formatRecipe = (recipe) => {
    ...
    for (let i = 0; i < cleanRecipe.length; i++) {
      ...
      // Detect start of a new step with a number followed by a period, and skip duplicate numbers
      if (
        /\d/.test(char) &&
        cleanRecipe[i + 1] === "." &&
        (i === 0 || !/\d\./.test(currentStep.slice(-2)))
      ) {
        // Push the current step to the steps array if it exists
        if (currentStep) {
          steps.push(currentStep.trim().replace(/^[.,\s]+|[.,\s]+$/g, ""));
        }
        currentStep = ""; // Start a new step, ignoring the repeated number
      } else {
        currentStep += char; // Add character to the current step
      }
    }
    ...
    // Return formatted steps with numbering
    return steps.map((step, idx) => (
    ...
    <>
      ...
      <div className="pt-16">
        <div className="mt-8 mx-auto max-w-4xl px-4">
          ...
          <ul className="grid grid-cols-3 gap-4">
            {predictedFoods.map((food, index) => (
              <li key={index} className="w-full">
                <button
                  className="bg-red-500 bg-opacity-70 shadow-lg rounded-lg w-[200px] h-30 p-3 shadow-lg flex flex-col items-center justify-center cursor-pointer"
          ...
          {selectedFood && (
            <div className="mt-8 bg-red-500 bg-opacity-70 shadow-lg rounded-lg p-6">
